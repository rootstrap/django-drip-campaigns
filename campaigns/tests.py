...
def _drips_generator(amount, **drip_extra_args):
    for i in range(amount):
        yield Drip(name="{}th drip".format(i), **drip_extra_args)
...
class DripsTestCase(TestCase):
    def test_campaings_creation(self):
        ...
        Drip.objects.bulk_create(_drips_generator(DRIP_AMOUNT, campaign=campaign))
    ...
    def test_remove_campaings(self):
        ...
        Drip.objects.bulk_create(_drips_generator(DRIP_AMOUNT, campaign=campaign_delete_drips))
        ...
        Drip.objects.bulk_create(_drips_generator(DRIP_AMOUNT, campaign=campaign_do_not_delete_drips))
